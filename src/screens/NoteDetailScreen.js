...
const NoteDetailScreen = ({ route, navigation }) => {
  ...
  const handleDelete = () => { /* ... as before ... */ };
  const handleShare = async () => { /* ... as before ... */ };
    ...
    <LinearGradient colors={[COLORS.gradientStart, COLORS.gradientEnd]} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Animated.View entering={FadeIn.delay(200)}>
          ...
          <Text style={styles.dateText}>Last updated on {formatDate(note.updatedAt)}</Text>
          ...
          {/* --- Restored Stats Section with new UI --- */}
          <View style={styles.statsSection}>
            <Text style={styles.sectionTitle}>Statistics</Text>
            <View style={styles.statsContainer}>
                <View style={styles.statItem}>
                    <Text style={styles.statValue}>{note.content.length}</Text>
                    <Text style={styles.statLabel}>Characters</Text>
                </View>
                <View style={styles.statItem}>
                    <Text style={styles.statValue}>{note.content.trim() ? note.content.trim().split(/\s+/).length : 0}</Text>
...
const styles = StyleSheet.create({
    ...
    tagsContainer: { flexDirection: 'row', flexWrap: 'wrap', marginBottom: 24, borderBottomWidth: 1, borderTopWidth: 1, borderColor: COLORS.border, paddingVertical: 16 },
    tag: { backgroundColor: COLORS.primary + '30', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 16, marginRight: 10, marginBottom: 10 },
    ...
    sectionTitle: { fontSize: 14, fontWeight: 'bold', color: COLORS.textSecondary, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 },
    statsContainer: { flexDirection: 'row', justifyContent: 'space-around', backgroundColor: COLORS.surface, borderRadius: 16, padding: 20 },
